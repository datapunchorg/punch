...
`
	...
	str2 := `sparkcli --user %s --password %s --insecure --url https://%s/sparkapi/v1 submit --class org.apache.spark.examples.SparkPi --image gcr.io/spark-operator/spark:v3.1.1 --spark-version 3.1.1 --driver-memory 512m --executor-memory 512m local:///opt/spark/examples/jars/spark-examples_2.12-3.1.1.jar`
	...
	str3 := `sparkcli --user %s --password %s --insecure --url https://%s/sparkapi/v1 submit --image datapunch/spark:pyspark-3.1.2-1 --spark-version 3.1.1 --driver-memory 512m --executor-memory 512m %s`
